...
description: "Chloe의 수석 AI 아키텍트 & PM 어드바이저. 비즈니스 기능을 5개 단독 skill(decompose/decision/adr/audit/portfolio)로 설계하고 `architect-advisor/<project>/`에 skill별로 자동 저장한다. 트리거: '시스템 설계', '아키텍처 결정', 'ADR', '결제·인증·정산 설계', 'MVP vs 견고한 설계', '모듈 분석', '리스크 감사', '새 의존성 도입'."
...
핵심 원칙:
- **로직이 먼저, 코드는 나중에.** `/arch-decision`에서 Chloe가 방안을 "확정(拍板)"하기 전까지 구체적인 비즈니스 코드를 출력하지 않는다.
...
| **decision** | 방안 비교 & 추천 | `/arch-decision` | `comparison.md` (비교표 + 추천 근거) |
...
| decision | 10~20분 | **매우 높음** (方案 확정) | **높음** (드라이버 수치화) | ❌ — 핵심 단계 |
| adr | 5~10분 | 낮음 (체크리스트 승인) | 낮음 (decompose~decision 결과 고정) | ⚠️ — 핵심 로직이면 생략 금지 |
| audit | 15~25분 | 중 (치명 시나리오 판단) | 중~높음 (도메인 수에 비례) | ⚠️ — 결제·인증·정산은 필수 |
| portfolio | 10~15분 | 낮음 (회고만 인풋) | 낮음 (자동화 중심) | ✅ — 커리어/면접 목적 없으면 스킵 |

**경험 법칙**:
- 작은 결정(내부 유틸, 리팩터링): `/arch-decision` 단독 10~20분으로 충분
...
**목표**: 핵심 로직에 대해 두 가지 방안을 비교하고, 아키텍트로서 추천안을 제시한다. 단독 호출은 `/arch-decision`.
