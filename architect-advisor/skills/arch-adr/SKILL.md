...
1. `architect-advisor/adrs/` (단일 product 표준 — repo-wide 누적)
2. `architect-advisor/<product>/adrs/` (monorepo product별)
3. 레거시: project-first `architect-advisor/<slug>/adrs/` → `architect-advisor/<slug>/adr/` → `docs/decisions/` → `adr/` → `docs/adr/` → `decisions/`

구버전 `phase3-adr/`, `phase2.5-adr/`가 남아 있으면 자동으로 `adrs/`로 이름이 바뀐다.

> **왜 ADR만 root-flat인가**: ADR은 *저장소 전체에 걸쳐 시간순으로 누적되는 결정 이력*이다. 한 프로젝트의 결정이 다음 프로젝트에 supersedes/superseded_by로 연결되는 일이 잦으므로, project 서브디렉토리로 분리하면 cross-project 인용이 깨진다. council/audit/decompose 같은 *프로젝트 내부 작업 산출물*은 반대로 `<skill>/<project>/`로 격리한다.
