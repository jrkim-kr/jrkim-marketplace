...
5. **증거 등급 재계산**: 사례를 append한 뒤 근거 ERR 수를 다시 세어 등급을 갱신한다
   - 잠정(2~4건) → 5건 도달 시 **확립으로 승격**. 등급이 적히는 **네 자리를 모두** 고치고 Changelog에 남긴다

     | 자리 | 읽는 이 |
     |---|---|
     | `<!-- evidence: 확립 \| ERR 근거 N건 -->` 주석 | 기계 |
     | 제목 뒤 `` `[확립]` `` 배지 | 목차를 훑는 사람 |
     | `**증거 등급**:` 산문 줄 | 그 모드를 읽는 사람 |
     | `to-tickets용 빠른 체크리스트` 표의 칸 | 티켓을 쓰는 사람 |

     뒤의 둘을 빠뜨려 **확립 10개가 참고로만 주입된다고 적혀 있었고 체크리스트 32칸이 잠정 쪽에 앉아 있었다**(#1329). 승격은 블록에만 반영되고 표는 뒤처지기 쉬우므로, 그 모드가 적힌 **모든 줄**을 찾아 옮긴다
...
- **증거 등급 줄은 등급에서 기계로 나오는 꼴**: `**증거 등급**:` 산문은 아래 두 꼴 중 하나로만 쓴다. 승격 이력 같은 꼬리는 그 뒤에 덧붙인다

  ```
  **증거 등급**: 확립 (근거 N건) — to-tickets가 **필수** 수락 기준으로 주입한다
  **증거 등급**: 잠정 (근거 N건) — to-tickets가 **참고**로만 주입한다. 근거 5건이 되면 확립으로 승격
  ```

  **확립에 승격 조건 문장을 남기지 않는다.** 이미 지난 조건이고, 읽는 사람과 `to-tickets` 둘 다 그 모드를 잠정으로 읽는다
