...
#   ④ 물음·행동으로 적는 한국어 머리말 — 규범형 명사구(`해결책`) 대신 「고친 것」·
#      「배운 것」처럼 적는 저장소가 있다. energino 146건 실측: 이것이 빠져 **52건이
#      네 항목 모두 빈 채로** 통과하고 있었다(`parse_errors` 는 0이었다 — 못 읽은
#      것과 없는 것을 가르지 않는다). 별칭을 더해 52건 → 3건이 됐다.
FIELD_ALIASES = {
    "root_cause": ["근본 원인", "Root Cause", "원인", "Cause", "분석",
                   "근인", "根因", "根本原因", "무엇이 진짜 문제였나"],
    "affected_modules": ["영향 모듈", "Affected Modules", "Affected", "Impact",
                         "영향 범위", "관련 파일", "Related Files",
                         "영향 파일", "影响模块", "影响范围"],
    "solution": ["해결책", "Solution", "Fix", "해결 방법",
                 "해결", "처방", "修复", "解决", "解决方案",
                 "고친 것", "고침", "무엇을 고쳤나", "무엇을 바꿨나",
                 "고친 방식", "고친 방법", "어떻게 고쳤나"],
...
def extract_section(body: str, aliases: list[str]) -> str | None:
    ...
    for alias in aliases:
        # ## 헤더 탐색. 뒤에 붙는 두 가지를 허용한다:
        #   ① 괄호 보충 — "## 근본 원인 (Root Cause)"
        #   ② 줄표·쌍점 뒤의 한 줄 요약 — "## 근본 원인 — 이름을 잘못 물었다"
