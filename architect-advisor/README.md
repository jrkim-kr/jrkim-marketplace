...
- `arch-err-pattern` — `<ERR_DIR>/ERR-*.md` 횡단 분석으로 재발 충돌 패턴 추출 → `to-tickets`가 자동 참조하는 `CONFLICT_PATTERNS.md` 생성. ERR_DIR은 `.flushrc.json` → `find errors/` → `./errors/` 3단계 자동 해석 (W0.1, flush 플러그인과 동일 규칙).
